...
async function dispatch(request,response){
  try{
    ...
    if(isStaticPage(route.path)){
      await(route,request,response) //if static, no need to execute plugins (fast responses)
    }else{
...
async function dynamicPage(page,paramsREST,request,response){
  ...
  try{
    ...
    let sendResponse=prepareOutput(request,response);
    switch(request.method.toLowerCase()){
      case 'get':{
        let data=await service.get(request,response,params)
        sendResponse(null,data)
        break
      }case 'post':{
        let data=await service.post(request,response,params)
        sendResponse(null,data)
        break
      }case 'put':{
        let data=await service.put(request,response,params)
        sendResponse(null,data)
        break
      }case 'delete':{
        let data=await service.delete(request,response,params)
        sendResponse(null,data)
        break
      }default:{
        //
