...
function configure(customConfiguration) {
  ...
  pathRoot = path.dirname(process.argv[1])
...
async function start(customConfiguration) {
  try {
    ...
    logger = configuration.Logger.getLogger('a1server')
...
async function dynamicPage(page, paramsREST, request, response) {
  ...
  try {
    let fullResourcePath = path.join(pathRoot, configuration.dynamicFolder, resource + '.js')
