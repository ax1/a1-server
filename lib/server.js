...
async function start(customConfiguration) {
  try {
    ...
    setPerformance(configuration.performance)
 ...
 * Note: in production systems, performanace should be set to false 
 */
function setPerformance(bool) {
  if (bool) {
    configuration.Logger.configure(Logger.NoOutputLogger)
    ...
    if (isProxyPage(realPath, request)) {
      ...
      if (!configuration.externalBodyParser && (request.method === 'POST' || request.method === 'PUT' || request.method === 'DELETE' || request.method === 'PATCH')) {
        ...
        request.on('end', function () {
          request.body = body
          dynamicPage(realPath, route.params, request, response).catch(err => dispatchError(request, response, err))
        })
      } else {
...
function dispatchError(request, response, rawError) {
  const err = security.purify(rawError)
  const page = Number.isInteger(rawError) ? rawError.toString() : '500'
  const fn = getStaticPage(page) ? staticPage :
    getDynamicPage(page) ? dynamicPage :
      defaultErrorPage
