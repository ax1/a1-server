...
async function start(customConfiguration) {
  try {
    ...
    server.on('request', function(request, response) {
      logRequest(request)
      request.on('error', (request, response) => serverError(request, response, err))
...
async function dispatch(request, response) {
  try {
    ...
    logger.error(err)
    if (!response.finished) serverError(request, response, err)
    throw err
  }
}
 ...
 */
function isStaticPage2(page) {
  return hasExtension(page) ? true :
    getStaticPage(page) ? true : false
}

function isStaticPage(page) {
  if (hasExtension(page)) return true
  else {
    const p = getStaticPage(page)
    console.log(page + '    ' + p)
    return p ? true : false
  }
}
...
function isDynamicPage(page) {
  return getDynamicPage(page) ? true : false
}
...
async function dynamicPage(page, paramsREST, request, response) {
  ...
  try {
    ...
    if (fs.existsSync(fullResourcePath) === false) {
      let fullPath = path.join(pathRoot, configuration.staticFolder, page)
      if (fs.existsSync(fullPath + '.html')) return staticPage(page + '.html', request, response) // server/test = server/test.html
      else return staticPage(page + '/' + configuration.welcomePage, request, response) // server/test = server/test/index.html
    }
...
function serverError(request, response, err) {
  ...
  //if custom error page send it, else send a standard error proxyPage
  dynamicPage('500', null, request, response)
    .catch(err => {
