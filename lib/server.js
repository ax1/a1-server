...
async function start(customConfiguration) {
  try {
    ...
    server.on('request', process.env.SERVER_PERFORMANCE ? dispatcherPerf : dispatcherNormal)
...
function setLogger(customConfiguration) {
  ...
  else if (customConfiguration && customConfiguration.Logger) configuration.Logger.configure(customConfiguration.Logger)
