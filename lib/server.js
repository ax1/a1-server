...
async function start(customConfiguration) {
  try {
    ...
    server.on('request', function(request, response) {
      logRequest(request)
      request.on('error', (err) => serverError(request, response, err))
 ...
 * @param {router} route
 */
function addQueryStringParams(request, route) {
  const u = url.parse(request.url).pathname
  new url.URLSearchParams(u.search).forEach((value, key) => {
    if (route.params[key] == null) route.params[key] = value
  })
}
...
async function dispatchContent(route, request, response) {
  try {
    ...
    if (isProxyPage(realPath, request)) {
      ...
      addQueryStringParams(request, route)
...
function serverError(request, response, err) {
  const page = '500'
  const fn = isDynamicPage(page) ? dynamicPage :
    isStaticPage(page) ? staticPage :
    defaultErrorPage
