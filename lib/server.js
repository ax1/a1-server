...
const BLOCK_SIZE = 10_000 // any file bigger will be chunked
 ...
 * @param  {Object|int} customConfiguration either the port address, or object containing the properties to be overridden
 * @return {void}
 */
function configure(customConfiguration) {
  // Similar to Express, if PORT env, override defaults
  if (process.env.PORT) configuration.port = parseInt(process.env.PORT)

  // Similar to netcat, if port number first in arguments, override defaults, if not number, do nothing
  if (process.argv.length > 2) {
    const p = parseInt(process.argv[2])
    if (p) configuration.port = p
  }

  // If custom config object, override any default
...
async function dispatch(request, response) {
  try {
    ...
    if (isDynamicPage(route.path)) {
      await plugins.executeBefore(request, response) // standard plugins
      await plugins.executeMiddleware(request, response) // express-like middleware
...
async function getStaticPage(page, request) {
  ...
  try {
    ...
    if (stat.isFile()) return fullPath
      ...
      if (!request.url.endsWith('/')) throw 301  // if folder and no trail slash, redirect (otherwise index.html can be loaded but its related resources won't)
      const defaultPage = fullPath + '/' + configuration.welcomePage
      if (!fs.existsSync(defaultPage)) throw 404
      return defaultPage
    } else return null
  } catch (err) {
...
async function staticPage(page, request, response) {
  ...
  try {
    ...
    if (fullPath) {
      ...
      if (size > BLOCK_SIZE) fs.createReadStream(fullPath).pipe(response)
      else response.end(await fsp.readFile(fullPath))
    } else {
    ...
    if (err === 301) {
      // This happens when the path is a folder (but without the end slash).Three cases:
      //   - path is the real path. Ok to send the new location
      //   - path is not the real one (behind a proxy) but a header with real path is provided. Also OK to send location
      //   - path is not the real one (behind a proxy) but not header. We cannot differenciate from case 1 (the proxy may not add any header), so we send the location but it will not work
      let location = request.headers['X-Real-Path'.toLowerCase()] ?? request.url
...
async function dynamicPage(page, paramsREST, request, response) {
  ...
  const httpRequestMethod = request.method.toLowerCase()
  let method = service[httpRequestMethod]
  if (!method && httpRequestMethod === 'delete') method = service[httpRequestMethod] // if no delete impl, check if remove is implemented
  if (!method) throw (405)

  // process request
  params = addQueryStringParams(request, params)
  let data = await method(request, response, params)
  if (response.finished) return // the endpoint file could invoke req.end() inside the function
  let output = null
  if (data?.pipe) {
    ...
    if (data === null || data === undefined) {
      ...
      output = String(data) // numbers + true/false
    } else {
      output = data
    }
    if (!process.env.PERFORMANCE) {
      if (!response.getHeader('Content-Type')) response.setHeader('Content-Type', 'text/plain; charset=utf-8')
      await plugins.executeAfter(request, response)
    }
    // ARF 28-12-23 res.write() forces node to chunked by default. Do chunk only for big payloads.
    const size = output?.length
    if (size > BLOCK_SIZE || response.getHeader('Transfer-Encoding')) {
...
function preventUnhandledStreamCrash(data, response) {
  //DISABLE the following line FOR NOW. because if developer is good, the stream error management should be done ok (sending data and closing the connection etc) but if developer is an scatterbrain, the socket will remain be open. So for the server itself it is better for now to force closing the socket.
 ...
 * - /public/private/password.txt/../../private/password.txt
 */
...
function configureDefaultFatalErrors() {
  ...
  // for callbacks, EventEmitter and Stream unhandled errors (very common to forget adding the .on('error') implementation 
  ...
  // (not sure if useful, since eventEmitter is only taken by uncaughtException) https://nodejs.org/api/errors.html#errors_err_unhandled_error
