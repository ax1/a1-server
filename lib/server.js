  ...
  if (customConfiguration) {
    if (Number.isInteger(customConfiguration)) {
      ...
      for (let prop of Object.keys(customConfiguration)) {
        ...
        else configuration.rules = { ...customConfiguration[prop], ...configuration.rules }
      }
    }
  }
...
function dispatchError(request, response, error) {
  const err = security.purifyError(error, response)
  const page = response.statusCode.toString()
  const fn = getStaticPage(page) ? staticPage : getDynamicPage(page) ? dynamicPage : defaultErrorPage
