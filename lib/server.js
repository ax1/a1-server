...
module.exports = { start, startWebsocket, stop, use, Logger }
...
async function start(customConfiguration) {
  try {
    ...
    const dispatcherNoLog = (req, res) => dispatch(req, res)
    const dispatcherNormal = (req, res) => { logRequest(req); dispatch(req, res) }
    const dispatcherPerf = (req, res) => res.end('hello world')
    server.on('request', process.env.SERVER_PERFORMANCE ? dispatcherPerf : process.env.SERVER_NO_LOG ? dispatcherNoLog : dispatcherNormal)
...
function setLogger(customConfiguration) {
  if (process.env.SERVER_PERFORMANCE) configuration.Logger = Logger.NoOutputLogger
...
async function dynamicPage(page, paramsREST, request, response) {
  ...
  if (data && data.pipe) {
    ...
    if (!process.env.SERVER_PERFORMANCE) {
...
/**
 * Start a websocket server. This is  server for the 95% of cases.
 * For other scenarios, create a websocket and add link it to the HTTPServer manually.
 * @param {http.Server} server - The HTTP server
 * @param {Object} events ? - [optional] The handler functions for each event.
 *  If a handler os not provided, a default function is added.
 *  Websocket events: open, close, message, error, pong.
 *  Examples: {}, {message: function(msg){...}, close: function(msg){...}} 
 * @return {Promise<WebSocketServer>} 
 */
async function startWebsocket(server, events) {
  return websocket.startWebsocket(server, events)
}
...
function printStartLog(configuration) {
  ...
  logger.info(`server at ${protocol}://localhost:${configuration.port}`)
  logger.info('--------------------------------')
  logger.info('\x1b[0m')
}
